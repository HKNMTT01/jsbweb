...
export const defaultCmsBlocks: CmsBlock[] = [
  ...
  },
  {
    ...
    type: "textarea",
    value:
      "Delivering reliable water solutions and sustainable energy initiatives for Sabah.",
    style: {
      fontSize: "18px",
      lineHeight: "1.8",
    },
  ...
  },
  {
    ...
    type: "textarea",
    value:
      "JETAMA Sdn. Bhd. continues to support treated water supply and strategic infrastructure growth in Sabah.",
    style: {
      fontSize: "18px",
      lineHeight: "1.8",
    },
  ...
  },
  {
    ...
    type: "textarea",
    value:
      "Explore JETAMA Group services across water operations, renewable energy and technical facilities.",
    style: {
      fontSize: "18px",
      lineHeight: "1.8",
    },
...
export async function upsertRow<T extends { id?: number }>(
  table: string,
  row: T,
) {
  if (!supabase) {
...
export async function loadCmsBlocks(opts?: {
  draft?: boolean;
}): Promise<CmsBlock[]> {
  if (opts?.draft) {
...
export async function saveCmsBlock(block: CmsBlock) {
  if (!supabase) {
    ...
    const currentBlocks = savedDraft
      ? (JSON.parse(savedDraft) as CmsBlock[])
      : defaultCmsBlocks;

    const exists = currentBlocks.some((item) => item.id === block.id);
    const nextBlocks = exists
      ? currentBlocks.map((item) => (item.id === block.id ? block : item))
      : [block, ...currentBlocks];
...
export async function loadSiteSettings(opts?: {
  draft?: boolean;
...
export async function saveSiteSettings(settings: SiteSettings) {
  ...
  const { error } = await supabase
    .from("site_settings")
    .upsert({ id: "global", value: settings } as never);
