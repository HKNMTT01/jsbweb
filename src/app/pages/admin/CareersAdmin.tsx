...
export default function CareersAdmin() {
  ...
  async function saveCareer(e: React.FormEvent) {
    ...
    setRequirementsText("");
    setStatus("Career post saved. Application email link will open candidate mail app.");
    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-slate-500">Create clickable career boxes with full details and direct mail application links.</p>
        </div>
      ...
      <form onSubmit={saveCareer} className="mt-8 rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-xl backdrop-blur">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <input type="email" className="rounded-xl border px-4 py-3 md:col-span-2" placeholder="Application email e.g. hr@jetama.com.my" value={form.application_email} onChange={(e) => setForm({ ...form, application_email: e.target.value })} required />
        </div>
        ...
        <label className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#f8fbff] px-4 py-3 text-sm font-bold">
          <input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} /> Active / display on website
        </label>
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex flex-wrap gap-2">
                  ...
                  {!item.is_active && <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black uppercase text-slate-500">Hidden</span>}
                </div>
